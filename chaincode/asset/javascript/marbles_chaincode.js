...
let Chaincode = class {
  ...
  async initAsset(stub, args, thisClass) {
    ...
    let assetProperties = JSON.parse(args[2])
    let owner = args[3]
    let operationName= args[4]
    let performedBy = args[5]
    let orgName = args[6]
    ...
    if (assetState.toString()) {
      throw new Error('This asset already exists: ' + assetId);
    }

    let schemaAsbytes = await stub.getState(assetType); 
    if (!schemaAsbytes.toString()) {
      let jsonResp = {};
      jsonResp.Error = 'Schema does not exist: ' + assetType;
      throw new Error(JSON.stringify(jsonResp));
    }

    let schemaAsset = {};
    try {
      schemaAsset = JSON.parse(schemaAsBytes.toString()); //unmarshal
    } catch (err) {
      let jsonResp = {};
      jsonResp.error = 'Failed to decode JSON of: ' + assetType;
      throw new Error(jsonResp);
    }
    if (!(v.validate(assetProperties, schemaAsset.schema))){
    ...
    asset.owner = owner;
    asset.operationsPerformed = []
    asset.operationsPerformed.push({operationName: operationName,performedBy: performedBy,orgName: orgName,timestamp: timestamp })
  ...
  // ===============================================
  // initSchema - create a new schema
  // ===============================================
  async initSchema(stub, args, thisClass) {
    if (args.length != 2) {
      throw new Error('Incorrect number of arguments. Expecting 6');
    }
    // ==== Input sanitation ====
    ...
    // ==== Check if asset already exists ====
    let schemaState = await stub.getState(schemaName);
    if (schemaState.toString()) {
      throw new Error('This asset already exists: ' + schemaName);
    }
    ...
    // === Save asset to state ===
    await stub.putState(schemaName, Buffer.from(JSON.stringify(schemaJson)));
    
    console.info('- end init schema');
  }

  // ===============================================
  // updateSchema - create a new schema
  // ===============================================
  async updateSchema(stub, args, thisClass) {
    if (args.length != 2) {
      throw new Error('Incorrect number of arguments. Expecting 6');
    }
    // ==== Input sanitation ====
    console.info('--- start update Schema---')
    if (args[0].length <= 0) {
      throw new Error('1st argument must be a non-empty string');
    }
    if (args[1].length <= 0) {
    ...
    let schemaAsBytes = await stub.getState(schemaName);
    if (!schemaAsBytes|| !schemaAsBytes.toString()) {
    ...
    try {
      ...
      let jsonResp = {};
      jsonResp.error = 'Failed to decode JSON of: ' + schemaName;
      throw new Error(jsonResp);
    }
    ...
    schemaAsset.schema = schema;
   

    // === Save asset to state ===
    await stub.putState(schemaName, Buffer.from(JSON.stringify(schemaAsset)));
    
    console.info('- end update schema');
  }

  // ===============================================
  // readMarble - read a asset from chaincode state
  // ===============================================
  async readAsset(stub, args, thisClass) {
    ...
    let assetAsbytes = await stub.getState(assetId); //get the marble from chaincode state
  ...
  async readSchema(stub, args, thisClass) {
    if (args.length != 1) {
      throw new Error('Incorrect number of arguments. Expecting name of the marble to query');
    }
    ...
    let assetAsbytes = await stub.getState(schemaName); //get the marble from chaincode state
    if (!assetAsbytes.toString()) {
      let jsonResp = {};
      jsonResp.Error = 'Schema does not exist: ' + schemaName;
      throw new Error(JSON.stringify(jsonResp));
    }
    console.info('=======================================');
    console.log(assetAsbytes.toString());
  ...
  async delete(stub, args, thisClass) {
    ...
    let valAsbytes = await stub.getState(assetId); //get the marble from chaincode state
    let jsonResp = {};
    if (!valAsbytes) {
      jsonResp.error = 'asset does not exist: ' + assetId;
      throw new Error(jsonResp);
    ...
    try {
      ...
      jsonResp = {};
      jsonResp.error = 'Failed to decode JSON of: ' + assetJSON;
      throw new Error(jsonResp);
    }

    await stub.deleteState(assetId); //remove the asset from chaincode state
  ...
  async updateAsset(stub, args, thisClass) {
    ...
    let assetId = args[0];
    let properties = JSON.parse(args[1]);
    let owner = args[2]
    let operationName = args[3]
    let performedBy = args[4]
    let orgName = args[5]
    console.info('- start update asset ', assetId, properties);
    ...
    let updateAsset = {};
    try {
      updateAsset = JSON.parse(assetAsBytes.toString()); //unmarshal
    } catch (err) {
      let jsonResp = {};
    ...
    for (let i = 0; i<keys.length; i++){
      if(updateAsset.properties.hasOwnProperty(keys[i]) == true){
        updateAsset.properties[keys[i]] = properties[keys[i]]
      }
    }
    console.info(updateAsset);
    
    //updateAsset.properties = properties; //change the properties
    updateAsset.operationsPerformed.push({operationName: operationName,performedBy: performedBy,orgName: orgName,timestamp: Date.now() })
    updateAsset.owner = owner;

    let updateAssetasBytes = Buffer.from(JSON.stringify(updateAsset));
    await stub.putState(assetId, updateAssetasBytes); //rewrite the marble

    console.info('- end updateAsset (success)');
  }
  ...
  // ===========================================================================================
  async getAssetByRange(stub, args, thisClass) {
