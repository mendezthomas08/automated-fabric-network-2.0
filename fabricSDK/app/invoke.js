...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name, transientData) => {
    try {
        ...
        let message;
        if (fcn === "initAsset" ) {
            result = await contract.submitTransaction(fcn, args[0], args[1], args[2],args[3],args[4],username,org_name);
            message = `Successfully added the  asset with key ${args[0]}`

        } else if (fcn === "initSchema" ) {
            result = await contract.submitTransaction(fcn, args[0], args[1]);
            message = `Successfully added the  asset with key ${args[0]}`

        } else if (fcn === "updateSchema") {
            result = await contract.submitTransaction(fcn, args[0], args[1]);
            message = `Successfully updated asset ${args[0]}`
        } else if (fcn === "updateAsset") {
            result = await contract.submitTransaction(fcn, args[0], args[1],args[2],args[3],username,org_name);
            message = `Successfully updated asset ${args[0]}`
